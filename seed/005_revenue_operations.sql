...
-- Run after seed/004_commerce_intake.sql.
...
alter table public.job_posting_requests
  add column if not exists status text not null default 'new',
  add column if not exists reviewed_at timestamptz,
  add column if not exists reviewed_by uuid references public.profiles(id) on delete set null;
...
do $$ begin
  alter table public.job_posting_requests
    ...
    check (status in ('new','contacted','approved','rejected','published','closed'));
exception when duplicate_object then null; end $$;
...
do $$ begin
  alter table public.opportunity_submissions
    ...
    check (status in ('new','qualified','matched','rejected','closed'));
exception when duplicate_object then null; end $$;
